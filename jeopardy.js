...
async function fillTable() {
    ...
    for (let clueIdx = 0; clueIdx < NUM_CLUES_PER_CAT; clueIdx++) {
        let $tr = $('<tr>');
        for (let catIdx = 0; catIdx < NUM_CATEGORIES; catIdx++) {
            $tr.append($('<td>').attr('id', `${catIdx}-${clueIdx}`).text('Clue'));
        }
        $('#jeopardy tbody').append($tr);
    }
}

//timer for players to answer questions
let timeIntervalUp;
let secondsDown = 10;
...
function handleClick(evt) {
    ...
    new MyTimer(function (val) {
        ...
        if (val === 0) {
            alert('Stumped?? Click Clue to reveal answer.');
        }
    });
